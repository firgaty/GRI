...
class CorrTP4 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (com.equals("exemple")) {
            ...
            for (int u = 0; u < deg.length; ++u) {
                checkeq(deg[u], deg_exp[u], "le degré de "+ u);
            }
            ...
            for (int k = 1; k < deg.length; ++k) {
                int exp = (int) Math.round( Math.pow((double) k, - gamma)
                                            / sum * (double) n);
                checkapprox(nb[k], exp, "le nombre de sommets de degré "+ k);
            }
        } else {
    ...
    static void check(boolean test, String erreur) {
        if ( ! test ) {
            System.out.println("Problème : " + erreur);
            System.exit(1);
        }
    }
    ...
    static void checkeq(int val, int expected, String erreur) {
        if (val != expected) {
            System.out.println("Problème avec " + erreur +" : "
